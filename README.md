...
1. 데스크톱 앱 상단의 **[🖥️ 화면 공유]** 버튼을 누릅니다.
2. 휴대폰(또는 다른 기기)에서 같은 주소의 `/viewer` 페이지를 엽니다.
   (Tailscale이면 `https://100.x.x.x:8443/viewer`)
3. 앱 상단에 표시된 **6자리 시청 PIN**을 입력하면 PC 화면이 실시간으로 표시됩니다.
   - PIN은 실행할 때마다 새로 생성되며, IP당 10회 오입력 시 차단됩니다.
   - 휴대폰 카메라 방송도 같은 페이지에서 함께 볼 수 있습니다.
...
- 스트림 **시청**은 (1) 데스크톱 앱(localhost 토큰) 또는 (2) 6자리 시청 PIN을 아는
  기기만 가능합니다. 토큰·PIN 모두 실행 시마다 무작위로 새로 생성됩니다.
- PIN 오입력은 IP당 10회로 제한됩니다.
- 외부 접속은 Tailscale 네트워크(본인 계정 기기)로 한정하는 것을 권장합니다.
